    ...
    internal class ChiTietNhapcs01
    {
        ...
        public bool kiemtratontai(int sohd, string masp)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                {
                    cmd.CommandText = "kiemtratontaichitiet";
        ...
        public bool them_ChiTiet_PhieuNhap(int MaPN, string MaSP, decimal soLuong, decimal TGia)
        {
            
                string insert_command = "INSERT INTO tblChiTietHDNhapHang " +
                                  "VALUES (" + MaPN + ", '" + MaSP + "', " + soLuong + ", " + TGia +
                                   ")";
        ...
        public void hien_ChiTietPN(DataGridView dataGridView,int sohd)
        {
        ...
        public bool update_ChiTiet_PhieuNhap(int MaChiTietPN, string MaSP,
            float Soluong, float TGiaNhap)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
        ...
        public void layTensp(string masp,TextBox textBox)
        {
            using(SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                using(SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "lay_TenSP";
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@MaSP", masp);
                    cmd.Parameters.Add("@isExisting", SqlDbType.NVarChar, 50).Direction = ParameterDirection.Output;

                    cmd.ExecuteNonQuery();
                    string i = Convert.ToString(cmd.Parameters["@isExisting"].Value);
                    textBox.Text = i+"";
                    conn.Close() ;
                }
            }
        }
        public void timkiem_chitiet_HD_Nhap_duavaoMasp(int sohd,string sMasp, DataGridView dataGridView)
        {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "tim_kiem_masp_chitiet";
                        cmd.CommandType = CommandType.StoredProcedure;
