...
namespace BTL_Csharp_vs1._0
{
    public partial class ChiTietHDNhap : Form
    {
        ...
        // lấy mã hóa đơn từ hóa đơn nhập
        public ChiTietHDNhap(string data)
        {
            InitializeComponent();
            txtCodeHD.Text = data;
        }
        ...
        public void hienChiTiet()
        {
            ...
            using (SqlConnection conn = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("sp_HienChiTietHDTheoMa", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void insertCTHoaDonNhap()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ThemChiTietHDNhap", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        if (txtCodeSP.Text == "" || txtTenSP.Text == "" || txtDonGia.Text == "" || txtSoLuong.Text == "")
                        {
                        ...
                        cmd.Parameters.AddWithValue("@DonGiaNhap", double.Parse(txtDonGia.Text));
        ...
        public void xoaCTHDNhap()
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(constr))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_XoaChiTietHDNhap", conn))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.Parameters.AddWithValue("@MaHD", txtCodeHD.Text);
                            cmd.Parameters.AddWithValue("@MaTTSP", maTTSP);
        ...
        private FrmHoaDonNhap FrmHoaDonNhap = new FrmHoaDonNhap();

        public ChiTietHDNhap(FrmHoaDonNhap frmHoaDonNhap)
        {
            InitializeComponent();
            this.FrmHoaDonNhap = frmHoaDonNhap;
        }

        // in hóa đơn
        private void button1_Click(object sender, EventArgs e)
        {
            int sohd = int.Parse(txtCodeHD.Text);
            Crt_HoaDonNhap crt_HoaDonNhap = new Crt_HoaDonNhap(sohd);
            crt_HoaDonNhap.Show();
        }
