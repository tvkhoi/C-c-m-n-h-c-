...
namespace BTL_LTW.Manage.TrangChu
{
    public partial class trangchuControl : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string filePath = @"D:\Lập trình hướng sk\console\BTL_LTW\data_TrangChu.json";
                    ...
                    if (data != null && data.sanphamf != null)
                    {
                        for (int i = 0; i < data.sanphamf.Count; i++)
                        {
                            ...
                            if (hinhanh != null && ten != null && mota != null && gia != null && giagoc != null && giamgia != null)
                            {
                                ...
                                // Kiểm tra Application["danhsach_sanpham"] đã tồn tại chưa
                                if (Application["danhsach_sanpham"] == null)
                                {
                                    ...
                                    // Lưu DataTable vào Application
                                    Application["danhsach_sanpham"] = dataTable;
                                }
                                else
                                {
                                    // Nếu đã tồn tại, lấy DataTable từ Application
                                    DataTable dataTable = (DataTable)Application["danhsach_sanpham"];
                                    // Thêm dòng dữ liệu vào DataTable
                                    dataTable.Rows.Add(sp, hinhanh.Src, ten.InnerText, mota.InnerText, gia.InnerText, giagoc.InnerText, giamgia.InnerText, congketnoi, vixuly, card);
                                    // Lưu DataTable sau khi thêm dữ liệu vào Application
                                    Application["danhsach_sanpham"] = dataTable;
                                }
                            }
                            else
